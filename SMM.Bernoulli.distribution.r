##Smoothing mixture model using data with Bernoulli distribution
...
bmi_guts <- read.csv(file="simulated.bernoulli.csv", header=TRUE)
...
b1=gamm4(bmi ~ s(age),na.action=na.omit,  random = ~ (1|id), family=binomial(link = "logit"),  data=data1)  

b2=gamm4(bmi ~ s(age),na.action=na.omit,  random = ~ (1|id), family=binomial(link = "logit"),  data=data2)  
...
bmi_guts$y1_hat=exp(bmi_guts$p1)/(1+exp(bmi_guts$p1))
bmi_guts$y2_hat=exp(bmi_guts$p2)/(1+exp(bmi_guts$p2))

bmi_guts$var1_varying=((bmi_guts$y1_hat-bmi_guts$bmi)^2)/(bmi_guts$y1_hat*(1-bmi_guts$y1_hat))
bmi_guts$var2_varying=((bmi_guts$y2_hat-bmi_guts$bmi)^2)/(bmi_guts$y2_hat*(1-bmi_guts$y2_hat))
...
write.csv(LLK_2,file="Bernoulli_LLK_2.csv", na="", col.names =T, row.names=T)
write.csv(BIC_2,file="Bernoulli_BIC_2.csv", na="", col.names =T, row.names=T)
...
write.csv(class_2,file="Bernoulli_class_2.csv", na="", col.names =T, row.names=T)
...
class_pbmi$pbmi1=exp(class_pbmi$pbmi1_pre)/(1+exp(class_pbmi$pbmi1_pre))
class_pbmi$pbmi2=exp(class_pbmi$pbmi2_pre)/(1+exp(class_pbmi$pbmi2_pre))
...
jpeg('SMM_Bernoulli_2.png', width = 7, height = 7, units = 'in', res = 600)

par(mfrow=c(1,1))

plot(main="SMM_Bernoulli_2", out1_mean_avg$age, out1_mean_avg$pbmi_new, type="l", col="black", 
xlab="Time", ylab="Mean Y", ylim=range(0, 1.3),  xlim=range(0, 1))

lines(out2_mean_avg$age,out2_mean_avg$pbmi_new, type="l", col="red")

legend(0, 1.3, legend=c("Group 1", "Group 2"),
...
b3=gamm4(bmi ~ s(age),na.action=na.omit,   random = ~ (1|id), family=binomial(link = "logit"), data=data3)  
...
bmi_guts$y1_hat=exp(bmi_guts$p1)/(1+exp(bmi_guts$p1))
bmi_guts$y2_hat=exp(bmi_guts$p2)/(1+exp(bmi_guts$p2))
bmi_guts$y3_hat=exp(bmi_guts$p3)/(1+exp(bmi_guts$p3))

bmi_guts$var1_varying=((bmi_guts$y1_hat-bmi_guts$bmi)^2)/(bmi_guts$y1_hat*(1-bmi_guts$y1_hat))
bmi_guts$var2_varying=((bmi_guts$y2_hat-bmi_guts$bmi)^2)/(bmi_guts$y2_hat*(1-bmi_guts$y2_hat))
bmi_guts$var3_varying=((bmi_guts$y3_hat-bmi_guts$bmi)^2)/(bmi_guts$y3_hat*(1-bmi_guts$y3_hat))
...
write.csv(LLK_3,file="Bernoulli_LLK_3.csv", na="", col.names =T, row.names=T)
write.csv(BIC_3,file="Bernoulli_BIC_3.csv", na="", col.names =T, row.names=T)
...
write.csv(class_3,file="Bernoulli_class_3.csv", na="", col.names =T, row.names=T)
...
class_pbmi$pbmi1=exp(class_pbmi$pbmi1_pre)/(1+exp(class_pbmi$pbmi1_pre))
class_pbmi$pbmi2=exp(class_pbmi$pbmi2_pre)/(1+exp(class_pbmi$pbmi2_pre))
class_pbmi$pbmi3=exp(class_pbmi$pbmi3_pre)/(1+exp(class_pbmi$pbmi3_pre))
...
jpeg('SMM_Bernoulli_3.png', width = 7, height = 7, units = 'in', res = 600)

par(mfrow=c(1,1))

plot(main="SMM_Bernoulli_3", out1_mean_avg$age, out1_mean_avg$pbmi_new, type="l", col="black", 
xlab="Time", ylab="Mean Y", ylim=range(0, 1.3),  xlim=range(0, 1))

lines(out2_mean_avg$age,out2_mean_avg$pbmi_new, type="l", col="red")
lines(out3_mean_avg$age,out3_mean_avg$pbmi_new, type="l", col="green")

legend(0, 1.3, legend=c("Group 1", "Group 2", "Group 3"),
...
b4=gamm4(bmi ~ s(age),na.action=na.omit, random = ~ (1|id), family=binomial(link = "logit"), data=data4)  
...
bmi_guts$y2_hat=exp(bmi_guts$p2)/(1+exp(bmi_guts$p2))
bmi_guts$y3_hat=exp(bmi_guts$p3)/(1+exp(bmi_guts$p3))
bmi_guts$y4_hat=exp(bmi_guts$p4)/(1+exp(bmi_guts$p4))

bmi_guts$var1_varying=((bmi_guts$y1_hat-bmi_guts$bmi)^2)/(bmi_guts$y1_hat*(1-bmi_guts$y1_hat))
bmi_guts$var2_varying=((bmi_guts$y2_hat-bmi_guts$bmi)^2)/(bmi_guts$y2_hat*(1-bmi_guts$y2_hat))
bmi_guts$var3_varying=((bmi_guts$y3_hat-bmi_guts$bmi)^2)/(bmi_guts$y3_hat*(1-bmi_guts$y3_hat))
bmi_guts$var4_varying=((bmi_guts$y4_hat-bmi_guts$bmi)^2)/(bmi_guts$y4_hat*(1-bmi_guts$y4_hat))
...
write.csv(LLK_4,file="Bernoulli_LLK_4.csv", na="", col.names =T, row.names=T)
write.csv(BIC_4,file="Bernoulli_BIC_4.csv", na="", col.names =T, row.names=T)
...
write.csv(class_4,file="Bernoulli_class_4.csv", na="", col.names =T, row.names=T)
...
class_pbmi$pbmi2=exp(class_pbmi$pbmi2_pre)/(1+exp(class_pbmi$pbmi2_pre))
class_pbmi$pbmi3=exp(class_pbmi$pbmi3_pre)/(1+exp(class_pbmi$pbmi3_pre))
class_pbmi$pbmi4=exp(class_pbmi$pbmi4_pre)/(1+exp(class_pbmi$pbmi4_pre))
...
jpeg('SMM_Bernoulli_4.png', width = 7, height = 7, units = 'in', res = 600)

par(mfrow=c(1,1))

plot(main="SMM_Bernoulli_4", out1_mean_avg$age, out1_mean_avg$pbmi_new, type="l", col="black", 
xlab="Time", ylab="Mean Y", ylim=range(0, 1.3),  xlim=range(0, 1))
...
legend(0, 1.3, legend=c("Group 1", "Group 2", "Group 3", "Group 4"),
...
b5=gamm4(bmi ~ s(age),na.action=na.omit,  random = ~ (1|id), family=binomial(link = "logit"), data=data5)  
...
bmi_guts$y3_hat=exp(bmi_guts$p3)/(1+exp(bmi_guts$p3))
bmi_guts$y4_hat=exp(bmi_guts$p4)/(1+exp(bmi_guts$p4))
bmi_guts$y5_hat=exp(bmi_guts$p5)/(1+exp(bmi_guts$p5))

bmi_guts$var1_varying=((bmi_guts$y1_hat-bmi_guts$bmi)^2)/(bmi_guts$y1_hat*(1-bmi_guts$y1_hat))
bmi_guts$var2_varying=((bmi_guts$y2_hat-bmi_guts$bmi)^2)/(bmi_guts$y2_hat*(1-bmi_guts$y2_hat))
bmi_guts$var3_varying=((bmi_guts$y3_hat-bmi_guts$bmi)^2)/(bmi_guts$y3_hat*(1-bmi_guts$y3_hat))
bmi_guts$var4_varying=((bmi_guts$y4_hat-bmi_guts$bmi)^2)/(bmi_guts$y4_hat*(1-bmi_guts$y4_hat))
bmi_guts$var5_varying=((bmi_guts$y5_hat-bmi_guts$bmi)^2)/(bmi_guts$y5_hat*(1-bmi_guts$y5_hat))
...
write.csv(LLK_5,file="Bernoulli_LLK_5.csv", na="", col.names =T, row.names=T)
write.csv(BIC_5,file="Bernoulli_BIC_5.csv", na="", col.names =T, row.names=T)
...
write.csv(class_5,file="Bernoulli_class_5.csv", na="", col.names =T, row.names=T)
...
class_pbmi$pbmi3=exp(class_pbmi$pbmi3_pre)/(1+exp(class_pbmi$pbmi3_pre))
class_pbmi$pbmi4=exp(class_pbmi$pbmi4_pre)/(1+exp(class_pbmi$pbmi4_pre))
class_pbmi$pbmi5=exp(class_pbmi$pbmi5_pre)/(1+exp(class_pbmi$pbmi5_pre))
...
jpeg('SMM_Bernoulli_5.png', width = 7, height = 7, units = 'in', res = 600)

par(mfrow=c(1,1))

plot(main="SMM_Bernoulli_5", out1_mean_avg$age, out1_mean_avg$pbmi_new, type="l", col="black", 
xlab="Time", ylab="Mean Y", ylim=range(0, 1.3),  xlim=range(0, 1))
...
legend(0, 1.3, legend=c("Group 1", "Group 2", "Group 3", "Group 4", "Group 5"),
